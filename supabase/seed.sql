insert into public.programs (title, duration_days)
values ('Mind Power', 28)
on conflict (title) do update
set duration_days = excluded.duration_days;

-- Verified against the live public `mind-power-audio` bucket objects on 2026-03-16.
-- The bucket now uses public object URLs in the app, and `audio_path` stores the
-- canonical weekly session audio for each week.
-- These paths are case-sensitive and must match the exact storage object names:
...
--
-- Week 4 note:
-- The uploaded storage objects do not include a Week 4 file named
-- `Exercises for the Week`, so the current mapping preserves the existing
-- fallback to `Week 4/03 Contacting the Subconscious Mind.mp3`.

with base_program as (
  select id
...
select
  base_program.id,
  seeded_week.week_number,
  seeded_week.title,
  seeded_week.audio_path,
  seeded_week.exercise_text
from base_program
cross join (
  values
    (
...
7. Go over your notes for 5 to 10 minutes every day.$$ 
    ),
    (
      2,
      'Week 2',
...
8. Go over your notes for 5 minutes every day.$$ 
    ),
    (
      3,
      'Week 3',
...
7. Drink 2 to 3 litres of water every day.$$ 
    ),
    (
      4,
      'Week 4',
...
5. Project of the week: spend 10 minutes every day visualising and seeding whatever goal you wish.$$ 
    )
) as seeded_week (week_number, title, audio_path, exercise_text)
